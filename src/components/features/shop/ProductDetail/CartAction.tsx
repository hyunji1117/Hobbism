...
export default function CartAction({
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // 서버로 전송할 값 확인 (POST 전)
      console.log('보낼 장바구니 데이터:', body);

      // 서버에 API 요청 (POST)
      const result = await fetchAddToCart(body);

      // 장바구니 추가 API 응답 콘솔 확인 (응답 전체)
      console.log('장바구니 추가 API 응답:', result);

      // 추가 후 장바구니 전체 목록 fetch
      const updatedCart = await fetchCartList();

      // 받아온 장바구니 전체목록 콘솔에 출력 (검증)
      console.log('최신 장바구니 목록:', updatedCart);
